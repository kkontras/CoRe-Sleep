...
class MultilogueNetCell(nn.Module):
    def __init__(self, D_m_party, D_m_context, D_g, D_p, D_e, dropout=0.5):
        super().__init__()
    ...
    def forward(self, U, U_context, g_hist, q0, e0):
        g_ = self.g_cell(torch.cat([U_context, U], dim=1),
        ...
        qs_ = self.p_cell(U_c_.contiguous().view(-1, self.D_m_party + self.D_g), q0.view(-1, self.D_p)).view(
            U.size()[0], -1, self.D_p)
...
class MultilogueNet(nn.Module):
    ...
    def forward(self, U, U_context):
        g_hist = torch.zeros(0).type(U_context.type())
        e_ = torch.zeros(0).type(U.type())
        e = e_
        alpha = []
        for u_, uc_ in zip(U, U_context):
            g_, q_, e_, alpha_ = self.multilogue_cell(u_, uc_, g_hist)
...
class Multilogue_Net_CategoricalModel(nn.Module):
    def __init__(self, D_m_party_text, D_m_party_audio, D_m_context, D_g, D_p, D_e, D_h, n_classes=7,
    ...
    def forward(self, U_t, U_a, U_context):
        emotions_f_t, alpha_f_t = self.multilogue_net_f_t(U_t, U_context)
        emotions_f_a, alpha_f_a = self.multilogue_net_f_a(U_a, U_context)
        emotions_f_t = self.dropout_rec(emotions_f_t)
        emotions_f_a = self.dropout_rec(emotions_f_a)
        rev_U_t = self._reverse_seq(U_t)
        rev_U_a = self._reverse_seq(U_a)
        rev_U_context = self._reverse_seq(U_context)
        emotions_b_t, alpha_b_t = self.multilogue_net_r_t(rev_U_t, rev_U_context)
        emotions_b_t = self._reverse_seq(emotions_b_t)
        emotions_b_t = self.dropout_rec(emotions_b_t)
        emotions_t = torch.cat([emotions_f_t, emotions_b_t], dim=-1)
        emotions_b_a, alpha_b_a = self.multilogue_net_r_a(rev_U_a, rev_U_context)
        emotions_b_a = self._reverse_seq(emotions_b_a)
        ...
        log_prob = F.log_softmax(
            self.smax_fc(torch.cat([hidden_at(), hidden_t, hidden_a], dim=-1)), 2)
        return log_prob, alpha_f_t, alpha_b_t


class RegressionModel(nn.Module):
    def __init__(self, D_m_party_text, D_m_party_audio, D_m_party_video, D_m_context, D_g, D_p, D_e, D_h, D_a=100,
    ...
    def forward(self, U_t, U_a, U_v, U_context, qmask, umask):
        ...
        pred = 3 * torch.tanh(self.smax_fc(
            torch.cat([hidden_at(), hidden_va(), hidden_tv(), hidden_t, hidden_a, hidden_v], dim=-1)).squeeze())
