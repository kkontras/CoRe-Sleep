...
from graphs.models.xsleepnet.nn_basic_layers import *
from graphs.models.xsleepnet.filterbank_shape import FilterbankShape
from graphs.models.xsleepnet.ops import *

class Hydrid_Sleep_Net(object):
    ...
    def construct_all_cnn_net(self):
        ...
        with tf.device('/gpu:0'), tf.variable_scope("deep_epoch_rnn_layer") as scope:
            deep_fw_cell, deep_bw_cell = bidirectional_recurrent_layer_gru_new(self.config.deep_nhidden,
                                                                           self.config.deep_nlayer,
                                                                           input_keep_prob=self.dropout_rnn,
                                                                           output_keep_prob=self.dropout_rnn)
            self.deep_rnn_out, self.deep_rnn_state = bidirectional_recurrent_layer_output_new(deep_fw_cell,
                                                                                deep_bw_cell,
                                                                                deep_rnn_input,
                                                                                self.epoch_seq_len,
                                                                                scope=scope)
        ...
        self.deep_loss = 0
        with tf.device('/gpu:0'), tf.name_scope("deep-loss"):
            for i in range(self.config.epoch_seq_len):
        ...
        self.deep_accuracy = []
        # Accuracy
        with tf.device('/gpu:0'), tf.name_scope("deep-accuracy"):
            for i in range(self.config.epoch_seq_len):
    ...
    def construct_seqsleepnet(self):
        ...
        with tf.device('/gpu:0'), tf.variable_scope("seq_epoch_rnn_layer") as scope:
            seq_fw_cell2, seq_bw_cell2 = bidirectional_recurrent_layer_bn_new(self.config.seq_nhidden2,
                                                                  ...
                                                                  output_keep_prob=self.dropout_rnn)
            self.seq_rnn_out2, self.seq_rnn_state2 = bidirectional_recurrent_layer_output_new(seq_fw_cell2,
                                                                             seq_bw_cell2,
                                                                             seq_e_rnn_input,
                                                                             self.epoch_seq_len,
                                                                             scope=scope)
        ...
        with tf.device('/gpu:0'), tf.variable_scope("seq_output_layer"):
            for i in range(self.config.epoch_seq_len):
                seq_score_i = fc(tf.squeeze(self.seq_rnn_out2[:,i,:]),
                                ...
                                name="seq_output",
                                relu=False)
                #seq_score_i = tf.nn.log_softmax(seq_score_i, name='seq_softmax')
        ...
        self.seq_loss = 0
        with tf.device('/gpu:0'), tf.name_scope("seq-loss"):
            for i in range(self.config.epoch_seq_len):
        ...
        self.seq_accuracy = []
        # Accuracy
        with tf.device('/gpu:0'), tf.name_scope("seq-accuracy"):
            for i in range(self.config.epoch_seq_len):
    ...
    def construct_joint_branch(self):
        ...
        with tf.device('/gpu:0'), tf.variable_scope("joint_output_layer"):
            for i in range(self.config.epoch_seq_len):
                joint_score_i = fc(tf.concat([tf.squeeze(self.deep_rnn_out[:,i,:]), tf.squeeze(self.seq_rnn_out2[:,i,:])], 1),
                                ...
                                name="joint_output",
                                relu=False)
                #joint_score_i = tf.nn.log_softmax(joint_score_i, name='joint_softmax')
        ...
        self.joint_loss = 0
        with tf.device('/gpu:0'), tf.name_scope("joint-loss"):
            for i in range(self.config.epoch_seq_len):
        ...
        self.joint_accuracy = []
        # Accuracy
        with tf.device('/gpu:0'), tf.name_scope("joint-accuracy"):
            for i in range(self.config.epoch_seq_len):
    ...
    def all_convolution_block(self, input, name):
        ...
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE) as scope:
            ...
            ## enc ~ [3000x1, 1500x16, 750x32, 375x64, 188x128, 94x128, 47x256, 24x256, 12x512, 6x1024]
            # enc ~ [3000x1, 1500x16, 750x32, 375x32, 188x64, 94x64, 47x128, 24x128, 12x256, 6x512]
            for layer_idx, layer_depth in enumerate(self.g_enc_depths):
                ...
                h_i = leakyrelu(h_i)
                # last cnn layer dropput will be handled by the rnn input dropout
